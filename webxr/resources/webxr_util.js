...
function xr_promise_test(name, func, properties, glContextType) {
  promise_test(async (t) => {
    ...
    if (glContextType) {
      ...
      gl = canvas.getContext(glContextType);
    }
...
function xr_session_promise_test(
  ...
  function createGLContext(glContextType) {
    let canvas = document.createElement('canvas');
    document.body.appendChild(canvas);
    return canvas.getContext(glContextType, {alpha: false, antialias: false, ...glcontextProperties});
  }

  xr_promise_test(
    name + ' - webgl',
    (t) => { return runTest(t, createGLContext('webgl')); },
    properties);
  xr_promise_test(
    name + ' - webgl2',
    (t) => { return runTest(t, createGLContext('webgl2')); },
    properties);
}
